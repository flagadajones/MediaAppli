...
/*
 * Copyright (C) 2010 Teleal GmbH, Switzerland This program is free software: you can redistribute it and/or modify it
 * under the terms of the GNU Lesser General Public License as published by the Free Software Foundation, either version
 * 3 of the License, or (at your option) any later version. This program is distributed in the hope that it will be
...
public class MediaRenderer {
	...
	// public void setMediaPlayerService(MediaPlayerService mMediaPlayer) {
	// avTransport.getImplementation().setmMediaPlayer(mMediaPlayer);
	// renderingControl.getImplementation().setmMediaPlayer(mMediaPlayer);
	// }

	public MediaRenderer() {

        // The connection manager doesn't have to do much, HTTP is stateless
        LocalService connectionManagerService = binder.read(MyConnectionManagerService.class);
        connectionManager = new DefaultServiceManager(connectionManagerService) {
            @Override
...
LastChangeParser lastChangeParser =new AVTransportLastChangeParser();
        ...
        LocalService<MyAVTransportService> avTransportService = binder.read(MyAVTransportService.class);
		...
		LastChangeParser lastChangeParserRendering = new AVTransportLastChangeParser();

        // The Rendering Control just passes the calls on to the backend players
        LocalService<MyAudioRenderingControlService> renderingControlService = binder
                .read(MyAudioRenderingControlService.class);
        renderingControl = new MyServiceManager<MyAudioRenderingControlService>(renderingControlService,lastChangeParserRendering ) {
            @Override
            protected MyAudioRenderingControlService createServiceInstance() throws Exception {
                return new MyAudioRenderingControlService(renderingControlLastChange);
            }
        };
        renderingControlService.setManager(renderingControl);

        try {
            SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(Application.instance);
            String uuid=preferences.getString("uuid", UUID.randomUUID().toString());
           preferences.edit().putString("uuid", uuid).commit();
           

			device = new LocalDevice(new DeviceIdentity(new UDN(uuid)/*
																	 * fromString(
																	 * "Fj MediaRenderer"
																	 * +
																	 * android.os
					...
					new UDADeviceType("MediaRenderer", 1), new DeviceDetails(
							"Fj on "
									+ preferences.getString("nomRenderer",
											android.os.Build.DEVICE),// ModelUtil.getLocalHostName(false),
							new ManufacturerDetails("Fj",
									"http://flagadajones.fr/mediarenderer/"),
							new ModelDetails("Fj MediaRenderer", "Test", "1",
									"http://flagadajones.fr/mediarenderer/")),
    ...
    public void runLastChangePushThread() {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    while (true) {
                        if (((MyConnectionManagerService) connectionManager.getImplementation()).isConnected()) {
                            // These operations will NOT block and wait for network responses
                            LastChangeAwareServiceManager managerAvTransport = (LastChangeAwareServiceManager) avTransport
                                    .getService().getManager();
                            LastChangeAwareServiceManager managerRenderingControl = (LastChangeAwareServiceManager) renderingControl
                                    .getService().getManager();
                            
                            managerAvTransport.fireLastChange();
                            managerRenderingControl.fireLastChange();
                        }
	...
	public ServiceManager<MyConnectionManagerService> getConnectionManager() {
		return connectionManager;
	}

	public ServiceManager<MyAVTransportService> getAvTransport() {
		return avTransport;
	}

	public ServiceManager<MyAudioRenderingControlService> getRenderingControl() {
