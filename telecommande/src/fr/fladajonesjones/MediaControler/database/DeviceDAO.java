...
public class DeviceDAO {
    ...
    public ArrayList<UpnpDevice> getAllDevice() {
        Cursor c = maBaseDonnees.query(MySQLOpenHelper.TABLE_DEVICES, new String[] { MySQLOpenHelper.COLONNE_DEVICE_UDN,MySQLOpenHelper.COLONNE_DEVICE_TYPE}, null, null, null, null, null);
       // 
        if (c.getCount() == 0){
        ...
        do {
           ...
           if (c.getInt(1)==UpnpDevice.TYPE_SERVER)
               device=new UpnpServerDevice();
           else if (c.getInt(1)==UpnpDevice.TYPE_RENDERER)
    ...
    public long insertDevice(UpnpDevice device) {
        ...
        if(device instanceof UpnpServerDevice)
            type=UpnpDevice.TYPE_SERVER;
            else if(device instanceof UpnpRendererDevice)
            type=UpnpDevice.TYPE_RENDERER;
