...
public class UpnpTransformer {
    final private static Logger log = Logger.getLogger(UpnpTransformer.class.getName());
    ...
    public static Musique metaDataToMusique(String metaData) {
        if (metaData != null) {
            ...
            if (content.getFirstContainer() != null) {
                ...
                List<URI> listeAlbumArt = albumUpnp.getPropertyValues(DIDLObject.Property.UPNP.ALBUM_ART_URI.class);
                ...
                for (int i = 0; i < tracksUpnp.length; i++) {
                    ...
                    for (Res uri2 : pisteUpnp.getResources()) {
                        ...
                        if (uri2.getDuration() != null || !uri2.getDuration().equals(""))
            ...
            if (content.getItems().size() != 0) {
                ...
                List<URI> listeAlbumArt = trackUpnp.getPropertyValues(DIDLObject.Property.UPNP.ALBUM_ART_URI.class);
                for (URI uri2 : listeAlbumArt) {
    ...
    public static String albumToMetaData(Album album) {
        ...
        try {
            if (album.albumArt != null)
                albumUpnp.addProperty(new DIDLObject.Property.UPNP.ALBUM_ART_URI(new URI(album.albumArt)));
    ...
    private static MusicTrack pisteToMetaData(Piste piste, String albumName) {
        ...
        try {
            if (piste.albumArt != null)
                pisteUpnp.addProperty(new DIDLObject.Property.UPNP.ALBUM_ART_URI(new URI(piste.albumArt)));
        } catch (URISyntaxException e) {
            log.log(Level.SEVERE, e.getMessage(), e);
        }
        pisteUpnp.addResource(new Res("audio/mp3", null, piste.duree, null, piste.url));
